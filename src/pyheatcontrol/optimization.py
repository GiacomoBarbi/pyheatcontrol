...
def optimization_time_dependent(args):
    ...
    if rank == 0:
        print("\n" + "="*70)
        print("TIME-DEPENDENT OPTIMAL CONTROL (Segregated Approach)")
        print("="*70)
    ...
    if rank == 0:
        print(f"[SETUP] Domain: {L*100:.1f}cm x {L*100:.1f}cm")
        print(f"        Time: {T_final_time}s, dt={dt}s, steps={num_steps}, Nt={Nt}")
        print(f"        Physical: k={k_val}, rho={rho}, c={c}")
        print(f"        T_ambient={T_ambient}°C, T_cure={T_cure}°C")
    ...
    if rank == 0:
        print("[DEBUG] constraint_boxes =", constraint_boxes, flush=True)

    if rank == 0:
        print("\n[CONTROL ZONES]")
        print(f"  Boundary Dirichlet: {len(ctrl_dirichlet)}")
        for i, seg in enumerate(ctrl_dirichlet):
            print(f"    {i+1}. {seg}")
        print(f"  Boundary Neumann: {len(ctrl_neumann)}")
        for i, seg in enumerate(ctrl_neumann):
            print(f"    {i+1}. {seg}")
        print(f"  Distributed boxes: {len(ctrl_distributed_boxes)}")
    ...
    # Aggiungi:
    if rank == 0:
        print("\n[DEBUG-MESH] Domain bounds:")
        print(f"  x: [0, {L}]")
        print(f"  y: [0, {L}]")
        print("  Target zones in physical coords:")
        for i, (xmin, xmax, ymin, ymax) in enumerate(target_boxes):
            print(f"    Zone {i+1}: x=[{xmin*L}, {xmax*L}], y=[{ymin*L}, {ymax*L}]")
    ...
    if rank == 0 and solver.n_ctrl_dirichlet > 0:
        ...
        print(f"[INIT-DEBUG] uD_Dirichlet(t=0) global min/max = {uD_test.x.array.min():.6e}, {uD_test.x.array.max():.6e}")
        print(f"[INIT-DEBUG] uD_Dirichlet(t=0) on ΓD min/max = {uD_test.x.array[dofs_test].min():.6e}, {uD_test.x.array[dofs_test].max():.6e}")
    ...
    if rank == 0 and solver.n_ctrl_distributed > 0:
        ...
        print(f"[INIT-DEBUG] uD(t=0) global min/max = {uD_test.x.array.min():.6e}, {uD_test.x.array.max():.6e}")
        print(f"[INIT-DEBUG] uD(t=0) on Ωc min/max = {uD_test.x.array[dofs_test].min():.6e}, {uD_test.x.array[dofs_test].max():.6e}")
        print(f"[INIT-DEBUG] Number of DOFs in Ωc: {len(dofs_test)}")
        print(f"[INIT-DEBUG] Total DOFs in V: {len(uD_test.x.array)}")
    # ============================================================
    # Initialize time-dependent multipliers for path constraints
    # ============================================================
    # Determine constraint time window
    ...
    if rank == 0:
        ...
        print(f"[PATH-CONSTRAINT] Steps: m ∈ [{sc_start_step}, {sc_end_step}] (of {num_steps})")
        if args.sc_lower is not None:
            print(f"[PATH-CONSTRAINT] Lower bound: T ≥ {args.sc_lower:.1f}°C")
        if args.sc_upper is not None:
            print(f"[PATH-CONSTRAINT] Upper bound: T ≤ {args.sc_upper:.1f}°C")

    # Initialize multipliers for each time step in constraint window (DG0)
    Vc = functionspace(domain, ("DG", 0))
    for m in range(num_steps + 1):  # Nt = num_steps + 1
        mu_L = Function(Vc)
        mu_U = Function(Vc)
        mu_L.x.array[:] = 0.0
        mu_U.x.array[:] = 0.0
    ...
    if rank == 0:
        print(f"[PATH-CONSTRAINT] Initialized {len(solver.mu_lower_time)} multiplier functions")
    ...
    if rank == 0:
        ...
        for i, marker in enumerate(solver.target_markers):
            ...
            print(f"\n[DEBUG-CELLS] Target zone {i+1}:")
            print(f"  Number of marked cells: {len(marked_cells_idx)}")
            if len(marked_cells_idx) > 0:
                centers = cell_centers[marked_cells_idx]
                print("  Cell centers (first 5):")
                for j, c in enumerate(centers[:5]):
                    print(f"    Cell {j}: ({c[0]:.4f}, {c[1]:.4f})")
    ...
    if rank == 0:
        print("\n[OPTIMIZATION]")
        print(f"  Scalar controls (u_controls): {n_ctrl_scalar} " f"(Dirichlet={solver.n_ctrl_dirichlet}, Neumann={solver.n_ctrl_neumann})")
        print(f"  Distributed controls (Function(V) per step): {solver.n_ctrl_distributed}")
        print(f"  Control time steps (intervals): {num_steps}  (State nodes Nt={Nt})")
        print(f"  Total variables: {n_vars_total}")
        print(f"  SC: beta={args.beta}, maxit={args.sc_maxit}")
        print(f"  Gradient descent: lr={args.lr}, inner_maxit={args.inner_maxit}")
        print(f"  Regularization: α_track={args.alpha_track}, α_u={args.alpha_u}, γ_u={args.gamma_u}")
        print("\n[DEBUG] Target zones cells:")
        for i, marker in enumerate(solver.target_markers):
            n_cells = np.sum(marker)
            print(f"  Zone {i+1}: {n_cells} cells marked")
    ...
    for sc_iter in range(args.sc_maxit):
        ...
        for inner_iter in range(args.inner_maxit):
            ...
            if solver.n_ctrl_neumann > 0:
                ...
                if solver.domain.comm.rank == 0:
                    dofs0 = solver.neumann_dofs[0]
                    q0 = u_neumann_funcs_time[0][0].x.array
                    print("[UPDATE-CHECK] q0_on_Gamma0 min/max:",
            ...
            if solver.n_ctrl_distributed > 0:
                ...
                if rank == 0:
                    ...
                    print("[UD-DEBUG] uD0(t0) min/max =",
                        float(a0.min()), float(a0.max()),
                        flush=True)

            if rank == 0 and solver.n_ctrl_neumann > 0:
                print("[Q-DEBUG] q(t0)   min/max =",
                    u_neumann_funcs_time[0][0].x.array.min(),
                    u_neumann_funcs_time[0][0].x.array.max(),
                    flush=True)
                print("[Q-DEBUG] q(tend) min/max =",
                    u_neumann_funcs_time[-1][0].x.array.min(),
                    u_neumann_funcs_time[-1][0].x.array.max(),
                    flush=True)
            ...
            if args.check_grad and sc_iter == 0 and inner_iter == 0:
                ...
                if rank == 0:
                    ...
                    print("[CHECK-GRAD] Done. Exiting now.", flush=True)
            ...
            if rank == 0:
                ...
                print("[U-UPDATE] grad        =", grad.copy())

            # u_unclipped = u_controls - args.lr * grad
            #
            # if rank == 0:
            #     print("[U-UPDATE] u_unclipped =", u_unclipped.copy())
            #
            # u_new = np.clip(u_unclipped, args.u_min, args.u_max)
            #
            # if rank == 0:
            #     print("[U-UPDATE] u_clipped   =", u_new.copy())
            #
            # u_controls = u_new
            ...
            if rank == 0:
                ...
                print(f"    Inner {inner_iter:2d}: J={J:.3e} (track={J_track:.3e}, "
        ...
        #         print(f"[DEBUG-RIDONDANZA] max|Y_inner - Y_outer| = {diff:.6e}", flush=True)

        if rank == 0:
            Vc = functionspace(domain, ("DG", 0))
            Tcell = Function(Vc); Tcell.interpolate(Y_all[-1])
            mask = solver.sc_marker.astype(bool)
            print("[SC-DEBUG] len(sc_marker) =", len(solver.sc_marker), flush=True)
        ...
        if rank == 0:
            print(f"[J-AFTER-MU] J={J_mu:.6e} (track={Jt_mu:.6e}, L2={JL2_mu:.6e}, H1={JH1_mu:.6e})", flush=True)
        ...
        if rank == 0:
            ...
            print(f"... μL_max={muL_max:.3e}, μU_max={muU_max:.3e}")

            if u_controls.size > 0:
                print(f"                u_mean={float(np.mean(u_controls)):.1f}, "
                    f"u_std={float(np.std(u_controls)):.1f}\n")
            else:
                print("                u_mean=n/a, u_std=n/a (no scalar controls)\n")
    ...
    if rank == 0:
        ...
        if solver.n_ctrl_distributed > 0:
            ...
            if rank == 0:
                print("\n" + "="*70)
                print("H1 TEMPORAL REGULARIZATION CHECK (DISTRIBUTED)")
                print("="*70)
                print(f"[H1t] gamma_u = {solver.gamma_u:.6e}, dt = {solver.dt:.6e}, num_steps = {num_steps}")
            ...
            for j in range(solver.n_ctrl_distributed):
                ...
                if rank == 0:
                    print(f"\n[H1t-DIST zone {j}]")
                    print(f"  predicted J_H1 = {pred_JH1:.6e}")
                    print(f"  roughness sqrt(sum ||Δu||^2) = {rough:.6e}")
                    print(f"  avg step L2-norm over Ωc (sqrt(mean ||Δu||^2)) = {avg_step:.6e}")
                    print(f"  max step ||Δu||^2 over Ωc = {max_step_L2:.6e}")
        ...
        if n_ctrl_total > 0:
            ...
            print(f"[DEBUG-EVOL] T_final_time={T_final_time}, dt={dt}, num_steps={num_steps}, "
            ...
            for ic in range(n_ctrl_total):
                ...
                for t_idx in idxs:
                    t_val = t_idx * dt
                    print(f"    t={t_val:8.1f}s: u={get_val(t_idx):.6e} {unit}")
                # held-last (ultimo controllo applicato)
                print(f"    t={T_final_time:8.1f}s: u={get_val(num_steps-1):.6e} {unit}  (held-last)")
    ...
    if rank == 0 and not args.no_vtk:
        print("\n" + "="*70)
        print("SAVING VISUALIZATION OUTPUT")
        print("="*70 + "\n")
